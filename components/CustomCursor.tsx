...
export default function CustomCursor() {
  ...
  // Decide once, on mount, whether this is a desktop fine-pointer session
  // that should get the custom cursor at all. matchMedia is only
  // meaningful client-side (no server render of a pointer/motion-
  // preference-dependent cursor to hydrate against), so this reveal has
  // to happen in an effect rather than an initializer.
  useEffect(() => {
  ...
  // Wire up positioning + event listeners only once the dot/ring elements
  // are actually mounted. They only render once `enabled` is true (see
  // the guard below), so this has to be a *separate* effect keyed on
  // `enabled` - reading the refs inside the same effect that sets
  // `enabled` would read them before React has committed the render that
  // creates the elements they point to, and silently no-op forever.
  useEffect(() => {
    if (!enabled) return;
